...
unsafeSplitAt n bs = (BSU.unsafeTake n bs, BSU.unsafeDrop n bs)
{-# INLINE unsafeSplitAt #-}

getFollowsBytesNum :: Word8 -> Int
...
getThreeCharsLen :: BS.ByteString -> (Int, Int, Int)
getThreeCharsLen bs =
  let len = BS.length bs in
    if | len == 0 -> (-1,-1,-1)
       | otherwise ->
         let fs = getFollowsBytesNum (BSU.unsafeHead bs) in
           if | len <= fs -> (fs, -1, -1)
              | otherwise ->
                let sn = getFollowsBytesNum $ BSU.unsafeIndex bs fs in
                  if | len <= fs+sn -> (fs, sn, -1)
                     | otherwise -> (fs, sn, getFollowsBytesNum $ BSU.unsafeIndex bs (fs+sn))
{-# INLINE getThreeCharsLen #-}

-- Data.Char.ord for ByteString
bsord :: BS.ByteString -> Int
bsord bs =
  let c = BSU.unsafeHead bs
  in
    if
      | c < toEnum 0x80
        -> toEnum $ fromEnum c
      | c < toEnum 0xe0
        -> let u = fromEnum $ c .&. 0x1F
               l = fromEnum $ BSU.unsafeIndex bs 1 .&. 0x3F
           in  unsafeShiftL u 6 .&. l
      | c < toEnum 0xf0
        -> let u = fromEnum $ c .&. 0x0F
               m = fromEnum $ BSU.unsafeIndex bs 1 .&. 0x3F
               l = fromEnum $ BSU.unsafeIndex bs 2 .&. 0x3F
           in unsafeShiftL u 12 .&. unsafeShiftL m 6 .&. l
      | c < toEnum 0xf8
        -> let
             fi = fromEnum $ c .&. 0x07
             sn = fromEnum $ BSU.unsafeIndex bs 1 .&. 0x3F
             th = fromEnum $ BSU.unsafeIndex bs 2 .&. 0x3F
             fo = fromEnum $ BSU.unsafeIndex bs 3 .&. 0x3F
           in
             unsafeShiftL fi 18
             .&. unsafeShiftL sn 12
             .&. unsafeShiftL th 6
...
data TokenizeState = TS { remain :: {-# UNPACK #-} !BS.ByteString
                        , tokenLength :: {-# UNPACK #-} !Int -- also index of byte now looking, w3's first byte
                        , score :: {-# UNPACK #-} !Int
                        , p1 :: !Marker
                        , p2 :: !Marker
                        , p3 :: !Marker
...
initialState text =
  let (lena, lenb, lenc) = getThreeCharsLen text
      one | lena > 0     = bsord $ BSU.unsafeTake lena text
          | otherwise    = e1
      two | lenb > 0     = bsord . BSU.unsafeTake lenb $
                             BSU.unsafeDrop lena text
          | one == e1    = e2
          | otherwise    = e1
      three | lenc > 0   = bsord . BSU.unsafeTake lenc $
                             BSU.unsafeDrop (lena+lenc) text
            | two == e2  = e3
            | two == e1  = e2
            | otherwise  = e1
  in  TS { remain      = text
...
moveNext = do
  ...
  let newW6Len | BS.length remain > offset
                 = getFollowsBytesNum $ BSU.unsafeIndex remain offset
               | otherwise = -1
  let newW6 | newW6Len > 0 = bsord $ BSU.unsafeTake newW6Len $ BSU.unsafeDrop offset remain
            | w6 >= e2     = e3
            | w6 == e1     = e2
            | otherwise    = e1
  put $ s { score       = bias
...
isFinished = do
  remlen <- BS.length <$> gets remain
  toklen <- gets tokenLength
  return $ toklen == remlen
{-# INLINE isFinished #-}

tailToResult :: Monad m => StateT TokenizeState m (Maybe BS.ByteString)
tailToResult = do
  s@TS {..} <- get
  let remlen = BS.length remain
  if remlen > 0
    then do
      put $ s { remain = BS.empty, tokenLength = 0 }
      return $ Just remain
    else return Nothing
{-# INLINE tailToResult #-}

evalScore :: Monad m => StateT TokenizeState m (Maybe BS.ByteString)
evalScore = do
  s@TS {..} <- get
  if score > 0
    then do
      let (word, newRem) = unsafeSplitAt tokenLength remain
      put $ s { remain = newRem
              , tokenLength = 0
              , p1    = p2
              , p2    = p3
...
evalOneStep :: Monad m => StateT TokenizeState m (Maybe BS.ByteString)
evalOneStep = do
  moveNext
  updateScore
  evalScore
...
tokenizeM :: StateT TokenizeState Identity (Maybe BS.ByteString, Bool)
tokenizeM = do
  flag <- isFinished
  if flag
    then do
...
tokenize :: BS.ByteString -> [BS.ByteString]
tokenize text =
...
tokenizeToVecM :: StateT TokenizeState (ST s) (MV.MVector s BS.ByteString, Int)
tokenizeToVecM = do
...
tokenizeToVec :: BS.ByteString -> V.Vector BS.ByteString
